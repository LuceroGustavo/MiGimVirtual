...
import com.migimvirtual.dto.CalendarioSemanalDTO;
import com.migimvirtual.servicios.CalendarioService;
import com.migimvirtual.servicios.CalendarioExcepcionService;
...
public class CalendarioController {
    ...
    public String calendarioSemanalProfesor(@PathVariable Long profesorId,
        ...
        // índice)
        java.util.List<com.migimvirtual.enums.DiaSemana> diasSemana = java.util.Arrays
                .asList(com.migimvirtual.enums.DiaSemana.values());
        model.addAttribute("diasSemana", diasSemana);

        // Preparar lista de listas de slots por día (en el mismo orden que diasSemana)
        java.util.List<java.util.List<com.migimvirtual.dto.CalendarioSemanalDTO.SlotHorarioDTO>> slotsPorDiaList = new java.util.ArrayList<>();
        int totalFilas = 0;
        for (com.migimvirtual.enums.DiaSemana dia : diasSemana) {
            java.util.List<com.migimvirtual.dto.CalendarioSemanalDTO.SlotHorarioDTO> slotsList = calendario.getSlotsPorDia()
                    .get(dia);
            if (slotsList == null)
        ...
        for (int diaIdx = 0; diaIdx < slotsPorDiaList.size(); diaIdx++) {
            ...
            java.util.List<com.migimvirtual.dto.CalendarioSemanalDTO.SlotHorarioDTO> slotsDelDia = slotsPorDiaList.get(diaIdx);
            if (slotsDelDia == null) continue;
            for (com.migimvirtual.dto.CalendarioSemanalDTO.SlotHorarioDTO slot : slotsDelDia) {
        ...
        for (int i = 0; i < totalFilas; i++) {
            String hora = "";
            for (java.util.List<com.migimvirtual.dto.CalendarioSemanalDTO.SlotHorarioDTO> slots : slotsPorDiaList) {
    ...
    public java.util.List<Usuario> obtenerUsuariosEnHorario(@RequestParam String dia,
        ...
        return calendarioService.obtenerUsuariosEnHorario(
                com.migimvirtual.enums.DiaSemana.valueOf(dia),
    ...
    public String actualizarCapacidadMaxima(@RequestParam int capacidadMaxima, @RequestParam Long profesorId) {
        ...
        for (com.migimvirtual.enums.DiaSemana dia : com.migimvirtual.enums.DiaSemana.values()) {
